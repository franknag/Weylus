...
pub fn allow_sleep() {
    let NSActivityIdleDisplaySleepDisabled = 1u64 << 40;
    let NSActivityUserInitiatedAllowingIdleSystemSleep = 0x00FFFFFFu64 | !NSActivityUserInitiatedAllowingIdleSystemSleep;
...
pub fn run(config: &Config, log_receiver: mpsc::Receiver<String>) {
    ...
    let mut check_native_hw_accel = CheckButton::default()
        .with_size(70, height)
        .below_of(&check_auto_start, 0);

    #[cfg(target_os = "linux")]
    {
        check_native_hw_accel.set_label("Hardware Acceleration");
