...
// via https://github.com/rennis250/processing-rs/blob/master/pri.c
// Original comment: Obtained from the Stage psychophysics suite for Matlab.
...
void setMaxPriority() {
    ...
    result = thread_policy_set(1);
    if (result) {
        printf("Failed to set max priority\n");
    }
}

void setDefaultPriority() {result = thread_policy_set(0);
    if (result) {
        printf("Failed to set max priority\n");
    }
}
